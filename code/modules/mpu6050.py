...
class mpu6050(threading.Thread):
    ...
    def __init__(self, address=0x68, bus=1, calibrate=True):
    ...
    def gyro_calibration(self):
        print('MPU6050: Calibration started')
        ...
        print('MPU6050: Calibration finished. X : {0}, Y : {1}, Z : {2}'
    ...
    def run_complementary_filter(self):
        print('MPU6050: Complementary filter started')
        alpha = 0.02
        dt = 0.01
...
if __name__ == "__main__":
    mpu = mpu6050()
    x = threading.Thread(target=mpu.run)
    x.start()
    

    print('Hello')
    for i in range(20):
        print(mpu.get_angles())
        time.sleep(0.5)
